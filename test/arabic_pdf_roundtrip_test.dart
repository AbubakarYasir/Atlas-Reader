...
void main() {
  ...
  setUp(() async {
    ...
    pdfPath = '${tempDir.path}${Platform.pathSeparator}arabic_scholarly_book.pdf';
  ...
  test('Stage 6: Arabic PDF Round-Trip with nested TOC, tags, and bi-directional sync', () async {
    ...
    expect(extractedInitial[1]['path'], ['أصول الفقه على منهج أهل السنة', 'المقدمة في تعريف أصول الفقه']);
    ...
    final importedCount = await db.syncBookmarkHierarchy(pdfPath, extractedInitial);
    expect(importedCount, 4);

    final dbBookmarks = await db.getBookmarksForFile(pdfPath);
    expect(dbBookmarks, hasLength(4));

    final parentBookmark = dbBookmarks.firstWhere((b) => b.title == 'أصول الفقه على منهج أهل السنة');
    final childBookmark = dbBookmarks.firstWhere((b) => b.title == 'المقدمة في تعريف أصول الفقه');
    ...
    final extSubtitles = [extRoot1[0].title, extRoot1[1].title, extRoot1[2].title];
    expect(extSubtitles, contains('المقدمة في تعريف أصول الفقه'));
    expect(extSubtitles, contains('الباب الأول: الأدلة الشرعية'));
    expect(extSubtitles, contains('الفصل الأول: القرآن الكريم والسنة النبوية - #دراسات, #فقه'));
    ...
    await fileSystem.writeAsBytesInBackground(pdfPath, modifiedBytes, flush: true);

    final externalBookmarks = await pdfEngine.extractBookmarks(pdfPath);
    expect(externalBookmarks.any((b) => b['title'] == 'خاتمة البحث والتوصيات'), isTrue);
    ...
    expect(syncedDbBookmarks.any((b) => b.title == 'خاتمة البحث والتوصيات'), isTrue);
  });
}
