...
void main() {
  ...
  test('Stage 7: Library database supports rich metadata, favorites, progress, and sorting', () async {
    ...
    final updatedHadith = (await db.getFilteredLibraryFiles()).firstWhere((b) => b.id == hadithBook.id);
    expect(updatedHadith.currentPage, 250);
    expect(updatedHadith.lastOpened, isNotNull);

    final authors = await db.getDistinctAuthors();
    expect(authors.keys, containsAll(['Imam Al-Bukhari', 'Ibn Khaldun', 'Imam Al-Shafi']));
    expect(authors['Imam Al-Bukhari'], 1);

    final tags = await db.getDistinctTags();
    expect(tags.keys, containsAll(['hadith', 'sunnah', 'history', 'sociology', 'law', 'usul']));

    final series = await db.getDistinctSeries();
    expect(series.keys, containsAll(['Kutub al-Sittah', 'Classical Thought']));
    ...
    final seriesFiltered = await db.getFilteredLibraryFiles(series: 'Classical Thought');
    expect(seriesFiltered, hasLength(2));

    final authorFiltered = await db.getFilteredLibraryFiles(author: 'Imam Al-Shafi');
    expect(authorFiltered, hasLength(1));
    expect(authorFiltered.first.title, 'Al-Risala');

    final sortedByTitle = await db.getFilteredLibraryFiles(sortBy: LibrarySortBy.title, ascending: true);
    expect(sortedByTitle.first.title, 'Al-Risala');
    expect(sortedByTitle.last.title, 'Sahih Al-Bukhari');

    final sortedByProgress = await db.getFilteredLibraryFiles(sortBy: LibrarySortBy.progress, ascending: false);
