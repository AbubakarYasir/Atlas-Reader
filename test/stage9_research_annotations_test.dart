...
void main() {
  ...
  test('Stage 9: Database and PDF engine create, persist, and extract standard PDF annotations', () async {
    ...
    final page1Annots = await db.getAnnotationsForFile(pdfPath, pageNumber: 1);
    ...
    expect(extracted.any((a) => a['text'] == 'الضروريات الخمس في مقاصد الشريعة'), isTrue);
    expect(extracted.any((a) => a['type'] == 'note'), isTrue);
  });

  test('Stage 9: Citation generator produces accurate APA, Chicago, MLA, and BibTeX citations', () {
    ...
    expect(apa, 'Ibn Khaldun. (1377). The Muqaddimah: An Introduction to History (p. 42). Princeton University Press.');
    ...
    expect(chicago, 'Ibn Khaldun. The Muqaddimah: An Introduction to History. Princeton University Press, 1377, p. 42.');
    ...
    expect(mla, 'Ibn Khaldun. The Muqaddimah: An Introduction to History. Princeton University Press, 1377, p. 42.');
