...
def find_link(links: list[dict[str, Any]], kind: str) -> list[dict[str, Any]]:
    ...
    return [
        item
        for item in links
        if str(item.get("url", "")) == "https://example.com/atlas-n2-fixture"
    ]
...
def validate_links(engine: str, evidence: dict[str, Any], failures: list[str], details: dict[str, Any]) -> None:
    ...
    for kind, expected in EXPECTED_A003.items():
        ...
        engine_details[kind] = {
            ...
            "expected": expected,
            "actual": [item.get("atlas_rect") for item in candidates],
        }
        if not matching:
...
def by_page(entries: list[dict[str, Any]]) -> dict[int, dict[str, Any]]:
    return {int(item.get("page", -1)): item for item in entries}
...
def validate_search(qt: dict[str, Any], pdfium: dict[str, Any], failures: list[str]) -> list[dict[str, Any]]:
    ...
    for page in range(3):
        q = qt_pages.get(page)
        p = pdfium_pages.get(page)
        if q is None or p is None:
            failures.append(f"a011-page-{page}:search-evidence-missing")
            continue

        expected_width, expected_height = EXPECTED_VISIBLE[page]
        for engine, item in (("qt-pdf", q), ("pdfium", p)):
            width = float(item.get("visible_width_points", -1))
            height = float(item.get("visible_height_points", -1))
        ...
        q_rect = union_rects(q.get("atlas_rects", []))
        p_rect = union_rects(p.get("atlas_rects", []))
        if q_rect is None or p_rect is None:
            failures.append(f"a011-page-{page}:normalized-search-rect-empty")
            continue
        ...
        comparison = {
            ...
            "edge_tolerance_points": SEARCH_EDGE_TOLERANCE,
        }
        ...
        for engine, rect in (("qt-pdf", q_rect), ("pdfium", p_rect)):
            if (
                rect["x"] < -0.1
                or rect["y"] < -0.1
                or rect["x"] + rect["width"] > expected_width + 0.1
                or rect["y"] + rect["height"] > expected_height + 0.1
            ):
...
def main() -> int:
    ...
    for engine, evidence in (("qt-pdf", qt), ("pdfium", pdfium)):
        ...
        if evidence.get("atlas_page_space") != ATLAS_PAGE_SPACE:
            failures.append(f"{engine}:page-space-contract-mismatch")

    link_details: dict[str, Any] = {}
    validate_links("qt-pdf", qt, failures, link_details)
    validate_links("pdfium", pdfium, failures, link_details)
    search_comparisons = validate_search(qt, pdfium, failures)

    result = {
        "schema": "atlas.n2.coordinate-normalization-validation.v1",
        "atlas_page_space": ATLAS_PAGE_SPACE,
        "link_tolerance_points": LINK_TOLERANCE,
        "search_edge_tolerance_points": SEARCH_EDGE_TOLERANCE,
        "checks": [
            "a003-internal-link-source-rectangle",
            "a003-external-link-source-rectangle",
            "a011-search-geometry-normal-page",
            "a011-search-geometry-cropped-page",
            "a011-search-geometry-rotated-page",
            "normalized-rectangles-in-effective-visible-bounds",
        ],
        "link_details": link_details,
        "search_comparisons": search_comparisons,
        "limitations": [
            "explicit destination /XYZ coordinates are not present in A003/A011 and remain a separate qualification subcase",
            "search rectangle comparison allows bounded metric differences between renderer text engines",
        ],
        "failures": failures,
        "passed": not failures,
    }
