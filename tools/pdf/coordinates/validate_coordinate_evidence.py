...
EXPECTED_A012_ATLAS = {1: {"x": 40.0, "y": 50.0}, 2: {"x": 250.0, "y": 40.0}}
EXPECTED_A012_QT_NATIVE = {1: {"x": 40.0, "y": 50.0}, 2: {"x": 40.0, "y": 50.0}}
QT_DESTINATION_LIMITATION = "requires-inherent-page-rotation-metadata"
...
def point_close(actual: dict[str, Any], expected: dict[str, Any], tolerance: float) -> bool:
    return close(actual.get("x", -999), expected["x"], tolerance) and close(actual.get("y", -999), expected["y"], tolerance)
...
def validate_links(engine: str, evidence: dict[str, Any], failures: list[str], details: dict[str, Any]) -> None:
    ...
    for kind, expected in EXPECTED_A003.items():
        ...
        engine_details[kind] = {"candidate_count": len(candidates), "matching_count": len(matching), "expected": expected, "actual": [item.get("atlas_rect") for item in candidates]}
        if not matching:
...
def validate_destinations(qt: dict[str, Any], pdfium: dict[str, Any], failures: list[str]) -> list[dict[str, Any]]:
    ...
    for page, expected_atlas in EXPECTED_A012_ATLAS.items():
        q_item = q.get(page); p_item = p.get(page)
        if q_item is None or p_item is None:
            failures.append(f"a012-page-{page}:destination-evidence-missing"); continue
        q_native = q_item.get("qt_destination_location", {})
        p_atlas = p_item.get("atlas_destination", {})
        expected_qt_native = EXPECTED_A012_QT_NATIVE[page]
        if not point_close(q_native, expected_qt_native, DESTINATION_TOLERANCE):
            failures.append(f"qt-pdf:a012-page-{page}:native-destination-unexpected")
        if q_item.get("atlas_destination_status") != QT_DESTINATION_LIMITATION:
            failures.append(f"qt-pdf:a012-page-{page}:limitation-status-missing")
        if not point_close(p_atlas, expected_atlas, DESTINATION_TOLERANCE):
            failures.append(f"pdfium:a012-page-{page}:destination-point-mismatch")
        comparisons.append({
            "destination_page": page,
            "expected_atlas": expected_atlas,
            "qt_native_location": q_native,
            "qt_atlas_status": q_item.get("atlas_destination_status"),
            "pdfium_atlas": p_atlas,
            "result": "qt-pass-with-limitation" if page == 2 else "pass-both",
        })
    return comparisons


def validate_search(qt: dict[str, Any], pdfium: dict[str, Any], failures: list[str]) -> list[dict[str, Any]]:
    qt_pages = by_page(qt.get("a011_search", [])); pdfium_pages = by_page(pdfium.get("a011_search", [])); comparisons: list[dict[str, Any]] = []
    for page in range(3):
        ...
        for engine, item in (("qt-pdf", q), ("pdfium", p)):
            ...
            if not close(width, expected_width, 0.05) or not close(height, expected_height, 0.05): failures.append(f"{engine}:a011-page-{page}:visible-size-mismatch")
        ...
        if any(value > SEARCH_EDGE_TOLERANCE for value in deltas.values()): failures.append(f"a011-page-{page}:cross-engine-search-geometry-mismatch")
        for engine, rect in (("qt-pdf", q_rect), ("pdfium", p_rect)):
            if rect["x"] < -0.1 or rect["y"] < -0.1 or rect["x"] + rect["width"] > expected_width + 0.1 or rect["y"] + rect["height"] > expected_height + 0.1: failures.append(f"{engine}:a011-page-{page}:normalized-search-rect-out-of-bounds")
    return comparisons


def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--qt", type=Path, required=True); parser.add_argument("--pdfium", type=Path, required=True); parser.add_argument("--output", type=Path); args = parser.parse_args()
    ...
    link_details: dict[str, Any] = {}; validate_links("qt-pdf", qt, failures, link_details); validate_links("pdfium", pdfium, failures, link_details)
    destination_comparisons = validate_destinations(qt, pdfium, failures); search_comparisons = validate_search(qt, pdfium, failures)
    result = {
        "schema": "atlas.n2.coordinate-normalization-validation.v3", "atlas_page_space": ATLAS_PAGE_SPACE,
        ...
        "checks": ["a003-internal-link-source-rectangle", "a003-external-link-source-rectangle", "a012-xyz-normal-destination", "a012-xyz-rotated-destination-candidate-limitation", "a011-search-geometry-normal-page", "a011-search-geometry-cropped-page", "a011-search-geometry-rotated-page", "normalized-rectangles-in-effective-visible-bounds"],
        "link_details": link_details, "destination_comparisons": destination_comparisons, "search_comparisons": search_comparisons,
        "limitations": ["Qt QPdfLink destination location on an inherently rotated target page is not rotated into Atlas effective-visible page space; structural page-rotation metadata or another destination provider is required", "search rectangle comparison allows bounded metric differences between text engines"],
